...
def extract_panels_with_gutters(image_path, output_dir="panels_fixed"):
    ...
    # --- Debug: show gutters in red ---
    debug = img.copy()
    debug[gutters > 0] = (0, 0, 255)  # red overlay
    cv2.imwrite("debug_gutters.png", debug)
    ...
    for i, (cnt, (x, y, w, h)) in enumerate(panels):
        ...
        cv2.imwrite(os.path.join(output_dir, f"panel_{i}.png"), rgba)

    print(f"✅ Extracted {len(panels)} panels into '{output_dir}', see 'debug_gutters.png' for gutter lines.")
