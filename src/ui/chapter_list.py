    ...
    def load_chapters_and_info(self):
        ...
        if not self.db_chapters:
            ...
            if series_path.is_dir():
                images = [p for p in series_path.iterdir() if p.is_file() and p.suffix.lower() in {'.png', '.jpg', '.jpeg', '.jpe', '.bmp', '.gif', '.webp', '.avif'}]
                if images:
        ...
        def on_finished(msg):
            ...
            try:
                ...
                if full_chapter_path.exists() and full_chapter_path.is_dir():
                    images = [str(p) for p in full_chapter_path.iterdir() if p.is_file() and p.suffix.lower() in {'.png', '.jpg', '.jpeg', '.jpe', '.bmp', '.gif', '.webp', '.avif'} and p.stem.lower() != 'cover']
