...
class AltManager:
    ...
    def load_alts(series_path: str) -> Dict[str, Dict[str, List[str]]]:
        ...
        with AltManager._cache_lock:
            if path_str in AltManager._cache:
                cached_mtime, cached_data = AltManager._cache[path_str]
        ...
        with AltManager._file_lock:
            try:
                with open(info_path, 'r', encoding='utf-8') as f:
                    data = json.load(f)
                    with AltManager._cache_lock:
                        AltManager._cache[path_str] = (mtime, data)
                    return data
